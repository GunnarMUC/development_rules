<?php
// Try connecting without password using socket authentication
try {
    $pdo = new PDO('mysql:host=localhost;dbname=vibe_templates', 'root', '', [
    ...
    // Also create the user if not exists and grant permissions
    try {
        $pdo->exec("CREATE USER IF NOT EXISTS 'vibe_template'@'localhost' IDENTIFIED BY '#Skilliks229!'");
        $pdo->exec("GRANT ALL PRIVILEGES ON vibe_templates.* TO 'vibe_template'@'localhost'");
        $pdo->exec("FLUSH PRIVILEGES");
        echo "✓ Database user permissions updated\n";
    } catch (PDOException $e) {
        echo "⚠ Could not update user permissions (may already exist)\n";
    }
